...
.. note::
   ...
   您必須擁有使用者帳號，且至少隸屬於一個專案（請參考 :ref:`creating_a_project` ），方能新增資料集。
...
「主題」與「專案」主要之差別在於，「主題」機制之劃分不具有資料權屬之特性（詳細可參考 :ref:`creating_a_project` ），亦即在本平台內一份資料集僅可屬於單一「專案」內，且該專案可以決定該資料集公開與否或專案內成員編輯權限；相較之下，「主題」僅將資料集進行分類，且一個資料集不限定僅屬於單一主題。
