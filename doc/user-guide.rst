...
.. note::
   ...
   您可以參考 `既有組織 <https://data.depositar.io/organization>`_ 填寫您的組織資訊。另外，根據管理員的設定，並非每個帳號都有建立新組織的權限，當您想建立新組織但不具權限時，您可以聯繫系統管理員。
...
.. note::
   ...
   受限 CKAN 權限機制，若欲加入組織的使用者尚未擁有帳號，請將其電子郵件寄送予平台管理員：data.contact AT depositar.io（AT 請代換為 @），由管理員進行邀請後，系統即會自動送出邀請信件予該使用者。
